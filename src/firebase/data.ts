...
export const saveSummarizedText = async (
  generatedText: SaveGeneratedTextInput
): Promise<void> => {
  const { currentUser } = useAuth.getState() as AuthSchema;
  ...
  // Get the current user's UID
  if (currentUserUid) {
    // Create a reference to the "summarized-text" collection
    const generatedTextRef = await addDoc(
      collection(db, "summarized-text"),
      generatedText
    );
    ...
    // Create a reference to the user's document in the "saved-generated" collection
    const userSavedPostsRef = doc(db, "saved-summarized", currentUserUid);
...
export const getSavedSummarizedTexts = async (
  dataSetter: React.Dispatch<React.SetStateAction<Array<object>>>,
  setLoaded: React.Dispatch<React.SetStateAction<boolean>>
) => {
  const { currentUser } = useAuth.getState() as AuthSchema;
